...
def create_ride():
        ...
        if(name in names and validate(timestamp) and int(source) >= 1 and int(source) <= 198 and int(destination) >= 1 and int(destination) <= 198):
            ...
            print(insert)
            r = requests.post('http://34.226.72.34:80/api/v1/db/write', json={"insert": insert,"column":"created_by,timestamp,source,destination","table":"ride","what":"insert"})
            res = jsonify()
            # res.statuscode = 201
            return res, 201
        else:
...
def list_rides(ride_id):
    try:
        ...
        f.close()
        ride_id = str(ride_id)  
        ride_ids = requests.post('http://34.226.72.34:80/api/v1/db/read', json={"table": "ride","columns":"ride_id","where":"source!='hasdfuhuhasujdhjkh'"})
        ...
        if(ride_id in ride_ids):
            result = requests.post('http://34.226.72.34:80/api/v1/db/read', json={"table": "ride","columns":"ride_id,created_by,timestamp,source,destination","where":"ride_id='"+ride_id+"'"})
            result1 = requests.post('http://34.226.72.34:80/api/v1/db/read', json={"table": "join_ride","columns":"username","where":"ride_id='"+ride_id+"'"})
...
def join_rides(ride_id):
    try:
        ...
        f.close()
        ride_id = str(ride_id)
        ride_ids = requests.post('http://34.226.72.34:80/api/v1/db/read', json={"table": "ride","columns":"ride_id","where":"ride_id!='2341356'"})
        ride_ids = ride_ids.json()
        print(ride_ids)
        ...
        if((username in names) and ((ride_id) in ride_ids)):
            ...
            requests.post('http://34.226.72.34:80/api/v1/db/write', json={"insert": insert,"column":"ride_id,username","table":"join_ride","what":"insert"})
            res = jsonify()
            # res.statuscode = 201
            return res, 200
        else:
...
def delete_ride(ride_id):
    try:
        ...
        if(int(ride_id) in ride_ids):
            requests.post('http://34.226.72.34:80/api/v1/db/write', json={"insert": "ride_id='"+ride_id+"'","column":"username,password","table":"ride","what":"delete"})
            res = jsonify()
            # res.statuscode = 201
            return res, 200
        else:
